...
## 801--Redis[远程字典服务器:Remote dictionary server]
...
| 根据时间（用户自选日、月度、年度、可选站点、桩名称）以高峰、尖峰平谷为时段，统计充电电量、电费、服务费、总费用、实收费用情况 | 峰平谷充电量统计 |
| ------------------------------------------------------------ | ---------------- |
...
```java
if (entity.getProtocolType == 1)
```

TOrderchargingRules.java:

订单规则，包含服务费abcd 的单价、金额；电费abcd单价、金额	
